...
- **Rendered Composition Realization (RCR)** — Render Gate measures row/column bands, spread, overlap, depth, focal dominance, media framing, and adjacent-scene geometry instead of trusting composition markers. Direction Fidelity also compares 360px with 1440px to verify the declared mobile transformation. [Metric and limitations](docs/RENDERED_COMPOSITION_REALIZATION.md).
...
- **Geometry over labels** — scene markers are joined to measured child geometry; correct metadata around a repeated centered stack no longer counts as realized composition.
- **Structure-sensitive evidence** — eight structural families use bounded profiles over bands, spread, overlap, depth, dominance, angular distribution, media framing, and internal pan.
- **Anti-convergence in the browser** — adjacent geometry below `0.12` is reported as repetition and penalizes the harmonic project score.
- **Responsive truth** — Direction Fidelity joins opaque scene hashes across 360px and 1440px and verifies the declared mobile transformation.
- **Privacy boundary** — preview reports contain categorical public structure names, hashes, and bounded numbers; no brief, copy, code, images, raw routes, or scene IDs are reported.
...
- **Source-to-render trace** — scene-bound evidence receives opaque `data-verve-evidence-id` hooks; static validation can use the existing Creative repair pass when hooks are missing.
- **Weighted realization** — supplied records and collection totals carry more weight than supporting facts, while comparison dimensions remain explicit decision evidence.
- **Prominence, not token count** — RES combines weighted coverage with bounded area, readable type scale, and semantic data structure. Empty, hidden, or page-sized proxy markers do not earn full credit.
- **First-viewport truth** — evidence assigned to the opening scene is measured separately, so an atmospheric opening cannot defer all supplied decision evidence below the fold.
- **Privacy-bounded receipts** — the workbench exposes RES beside FVE/FVF/DF, but the preview report contains only counts, unit scores, and hashes for missing evidence.

## What changed in 0.14 Brief Evidence Ledger

- **Verbatim evidence binding** — every extracted item carries its exact source offsets plus a digest of the originating brief; stale or modified source invalidates the contract.
- **Bilingual structured records** — English and Arabic briefs can provide product/service rows inside quotation marks or newline lists. Known labels and attributes remain distinct from a separately declared collection total.
- **Evidence-shaped scenes** — Story Graph scenes now choose information shapes such as record browser, evidence ledger, and comparison matrix from the actual brief instead of falling back to generic feature cards.
- **Honest incompleteness** — when a brief promises five lines but specifies only one, Verve records the 5/1 gap, renders the verified item, and requires a visible disclosure for the other four rather than synthesizing them.
- **Whole-project realization check** — deterministic validation measures record, attribute, comparison-dimension, gap-disclosure, and prohibited-pattern coverage across entry and supporting source files. This report is returned with code quality evidence and participates in repair/readiness.
...
[05] Production-minded entry code
        ↓
[05.5] Syntax + Brief Evidence Realization ─ deterministic in Fast / bounded repair in Creative
        ↓
[06] Distinctiveness + template diversity + FVE/FVF/RES/DF + engineering evidence
        ↓
