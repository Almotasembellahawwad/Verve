...
test("the demo workbench gives a laptop-width preview its own full row", async ({ page }, testInfo) => {
  ...
  const architectureFrame = page.frameLocator("iframe[title$='story preview']");
  ...
  await expect(page.frameLocator("iframe[title$='story preview']").locator(".sun")).toHaveCSS("display", "grid");
