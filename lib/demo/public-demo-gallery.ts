...
function receipt(input: Omit<PublicDemoReceipt, "engineVersion" | "assets" | "tests">): PublicDemoReceipt {
  return {
    engineVersion: "3.0.0-beta",
...
const architectureCssV2 = `${architectureCss}\n.hero-image{position:absolute;z-index:0;inset:10% 0 0 38%;margin:0;overflow:hidden;background:#9b9990}.hero-image:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,var(--concrete) 0%,transparent 34%,rgba(17,18,15,.08) 100%)}.hero-image img{width:100%;height:100%;object-fit:cover;filter:saturate(.76) contrast(1.05)}.hero-image figcaption{position:absolute;z-index:1;right:18px;bottom:16px;padding:8px 10px;background:rgba(17,18,15,.8);color:#f0f0e9;font-size:.625rem;letter-spacing:.08em;text-transform:uppercase}.hero .eyebrow,.hero h1,.hero-note,.down{position:relative;z-index:2}@media(min-width:761px){.hero h1{max-width:72%;font-size:clamp(4.8rem,9vw,8.5rem);line-height:.76;text-shadow:0 1px 0 var(--concrete)}.hero-note{position:absolute;right:4vw;width:min(280px,23vw);background:rgba(212,211,205,.92);padding:14px}.eyebrow{margin-bottom:6vh}}@media(max-width:760px){.hero-image{inset:31% 0 16% 22%}.hero-image:after{background:linear-gradient(180deg,var(--concrete),transparent 30%,rgba(17,18,15,.12))}.hero-image figcaption{max-width:170px}.hero-note{background:rgba(212,211,205,.9);padding:10px}}`;
