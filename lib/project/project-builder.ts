...
- Generated projects may use only bundled files or system stacks. Runtime font imports are not licensed by this manifest.
...
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after { scroll-behavior: auto !important; animation-duration: 0.01ms !important; animation-iteration-count: 1 !important; transition-duration: 0.01ms !important; }
  ...
  files.push(
    ...
    file("ASSETS.md", assetManifest(plan, assetDirection, assetDelivery), "markdown", "documentation"),
  );
...
export function buildGeneratedProject(
  ...
  const project = generated.framework === "html"
    ? htmlProject(name, generated, analysis, plan, assetDirection, assetDelivery)
    : generated.framework === "react"
      ? reactProject(name, generated, analysis, plan, assetDirection, assetDelivery)
      : nextProject(name, generated, analysis, plan, assetDirection, assetDelivery);
  const existingPaths = new Set(project.files.map((item) => item.path));
  project.files.push(...deliveredAssetFiles.filter((item) => !existingPaths.has(item.path)));
